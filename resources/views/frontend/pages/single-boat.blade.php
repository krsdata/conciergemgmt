...
@media only screen and (max-width: 800px) {
    ...
    <section class="position-relative boat-topname counting">
        <div class="container">
        <h2 class="unit-1-heading text-color" data-aos="fade-center" data-aos-duration="2000">{{ $page->page_name }}</h2>
        </div>
        </section>
    @if(!empty($slider))
    <!-- Header Slider Start !-->
            ...
            <div class="carousel-inner">
                @if(!empty($slider->slides))
                  ...
                  <div class="carousel-item @if($first == 0){{ 'active' }}@endif">
                      <img class="img-fluid w-100 lazyload" data-src="{{ $ss->image_path }}" alt="hamptons boat slides" style="max-height: 750px;">
    ...
    <div class="blog-item-content bg-white p-1 big-boat-detail">
        <button id="bookingWidget_trigger" class="nice-button-blue">Book now</button>
                                            <h3 class="boat-heading mb-4 pt-2">
                                                <strong>FROM ${{ substr($extra['price'], 0, -1) }}</strong> + {{ $extra['added_price'] }} <span class="place"><i class="fas fa-map-marker-alt"></i> {{ $extra['harbor'] }}</span>
                                            </h3>
                                            <div class="boat-info">
                                                <span class="mr-3">
                                                    <i class="fas fa-ruler-horizontal"></i>{{ $extra['size'] }} FEET
                                                </span>
                                                <span class="mr-3">
                                                    <i class="fas fa-user-friends"></i>{{ $extra['capacity'] }} PEOPLE
                                                </span>
                                                <span class="mr-3">
...
<section class="packageBox" id="packageBox">
  <div class="">
    ...
    <div class="row">
      <div class="col-md-12">
        <h1 class="big-heading" data-aos="fade-center" data-aos-duration="2000">COASTAL CONCIERGE PACKAGES</h1>
      </div>
  ...
  @foreach($headingcontent as $hh)
        @if($hh->heading == "Description")
...
<div class="col-md-12">
    ...
    <div class="row">
        <div class="col-md-6 ntbl">
           ...
           @foreach($headingcontent as $hh)
                @if($hh->heading == "Notable Features")
                    <div class="pb-5"><?php echo $hh->content; ?></div>
                @endif
            @endforeach
        </div>
...
<div class="col-md-12">
    ...
    @foreach($video as $vediocontent)
    <div class="row embed-responsive embed-responsive-16by9 mt-5 mb-3"><iframe width="1120" height="630" class="embed-responsive-item" src="{{ $vediocontent->video }}" frameborder="0" allowfullscreen="allowfullscreen"></iframe></div>
    @endforeach
</div>
</div>
</div>
...
<div class="col-md-12">
    ...
    @foreach($headingcontent as $hh)
        @if($hh->heading == "Departure Times")
            <?php echo $hh->content; ?>
        @endif
    @endforeach  
    <h3 class="sub-heading pt-5">DEPARTURE ADDRESS</h3>
    @foreach($headingcontent as $hh)
        @if($hh->heading == "Departure Address")
            <?php echo $hh->content; ?>
        @endif
    @endforeach  
    <h3 class="sub-heading pt-5">CRUISING MAP</h3>
    <p><img class="img-fluid mt-3 mb-3 lazyload cruising-area" data-src="public/photos/2/{{ $page->new_local_cruising_area }}" src="/public/photos/2/{{ $page->new_local_cruising_area }}" alt="coastal concierge boats local cruising area" /></p>
</div>
</div>
</div>
</div>    
...
<div class="col-md-12">
    ...
    @foreach($headingcontentTwo as $hh)
